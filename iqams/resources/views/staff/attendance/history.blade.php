<x-staff-layout title="Attendance History">
    <div class="space-y-5">
        ...
        <form class="grid gap-3 rounded-xl bg-white p-4 shadow-sm md:grid-cols-5">
            ...
            <select class="rounded-md border-gray-300" name="status"><option value="">All statuses</option>@foreach(['Completed','Incomplete','Absent'] as $value)<option @selected(request('status')===$value)>{{ $value }}</option>@endforeach</select>
