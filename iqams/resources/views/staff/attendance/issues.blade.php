<x-staff-layout title="Attendance Issues">
    <div class="mb-6">
                ...
                <div>
                    ...
                    <p class="text-sm text-slate-500">
                        ...
                        @elseif($day['isIncomplete'])
                            Missing:
                            {{ $day['events']->filter(fn($value) => !$value)->keys()->map(fn($value) => str($value)->replace('_', ' ')->title())->join(', ') }}@else{{ $day['notes']->join(', ') }}
                        @endif
                    </p>
                </div><span
                    class="self-start rounded-full bg-amber-50 px-3 py-1 text-sm text-amber-700">{{ $day['isIncomplete'] ? 'Incomplete' : $day['status'] }}
                    &middot; {{ $day['punctuality'] }}</span>
