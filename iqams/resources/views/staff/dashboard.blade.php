...
<x-staff-layout title="Dashboard">
    <div x-data="staffWorkspace" data-realtime-url="{{ route('staff.dashboard.realtime') }}"
        data-id-card-url="{{ route('id-card.show') }}">
                ...
                <div class="space-y-6">
                            ...
                            @foreach ([['percentage', 'Attendance rate', $totals['percentage'] . '%'], ['presentDays', 'Present', $totals['presentDays']], ['absentDays', 'Absent', $totals['absentDays']], ['inProgressCount', 'In Progress', $totals['inProgressCount']], ['incompleteCount', 'Incomplete', $totals['incompleteCount']], ['lateCount', 'Late', $totals['lateCount']], ['earlyOutCount', 'Early Out', $totals['earlyOutCount']]] as [$key, $label, $value])
                                <div class="px-5 py-5">
                                    <p class="text-xs font-medium uppercase tracking-wide text-gray-500">
                                        {{ $label }}</p>
                                    <p data-staff-stat="{{ $key }}"
                                        class="mt-2 text-2xl font-bold text-gray-900">{{ $value }}</p>
                                </div>
                            @endforeach
                        </div>
                    </section>
                    ...
                    <section class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
                        <div class="border-b border-gray-100 px-5 py-4">
                            ...
                            <p class="text-xs text-gray-500">Your latest recorded scans</p>
                        </div>
                        <div data-staff-recent class="divide-y divide-gray-100">
                            @forelse ($recentLogs as $log)
                                <article class="flex items-center gap-4 px-5 py-4">
                                    <div
                                        class="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-indigo-50 text-indigo-600">
                                        <x-heroicon-o-clock class="h-5 w-5" />
                                    </div>
                                    <div class="min-w-0 flex-1">
                                        <p class="text-sm font-semibold text-gray-800">
                                            {{ str($log->attendance_period ?? $log->attendance_type)->replace('_', ' ')->title() }}
                                        </p>
                                        <p class="text-xs text-gray-500">{{ $log->scan_time->format('F j, Y') }}</p>
                                    </div>
                                    <div class="text-right">
                                        <p class="whitespace-nowrap text-sm font-semibold tabular-nums text-gray-800">
                                            {{ $log->scan_time->format('g:i A') }}</p>
                                        <p class="text-xs capitalize text-gray-500">
                                            {{ str_replace('_', ' ', $log->punctuality_status ?? $log->status) }}</p>
                                    </div>
                                </article>
                            @empty
                                <div class="px-6 py-12 text-center text-sm text-gray-400">No attendance records yet.
                                </div>
                            @endforelse
                        </div>
                    </section>
                ...
                <aside class="space-y-6">
                    <section class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
                            ...
                            <div class="h-20 w-20 shrink-0 overflow-hidden rounded-full bg-indigo-100 text-indigo-700">
                                @if (Auth::user()->avatar_thumbnail_url)
                                    <img loading="lazy" width="80" height="80"
                                        src="{{ Auth::user()->avatar_thumbnail_url }}" alt="{{ $staff->fullName() }}"
                                    class="h-full w-full object-cover">@else<div
                                        class="grid h-full place-items-center font-bold">
                                        {{ strtoupper(substr($staff->first_name, 0, 1) . substr($staff->last_name, 0, 1)) }}
                                    </div>
                                @endif
                            </div>
                            <div class="min-w-0 text-sm leading-6">
                                <p class="break-words text-base font-extrabold text-gray-900">{{ $staff->fullName() }}
                                </p>
                                <p class="text-gray-600">Staff ID: {{ $staff->employee_no }}</p>
                                <p class="break-words text-gray-600">Office/Unit:
                                    {{ $staff->officeUnit?->name ?? 'N/A' }}</p>
                                <p class="text-gray-600">Position: Non-Teaching Personnel</p>
                            </div>
                        </div>
                        <div class="mt-5 text-center">
                            <p class="text-xs font-bold text-slate-800">My QR Code</p>
                            <div id="staff-qr"
                                class="mx-auto mt-2 grid min-h-44 min-w-44 w-fit place-items-center rounded-lg border border-slate-200 bg-white p-2 text-xs text-slate-500"
                                aria-label="Staff attendance QR code"></div>
                            <p class="mx-auto mt-2 max-w-[240px] text-[11px] leading-4 text-slate-500">Present this QR
                                code to the dedicated scanner for attendance.</p>
                        </div>
                        <div class="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-1">
                            ...
                            <button type="button"
                                @click="window.ensureIqamsQrCode().then(() => window.downloadIqamsIdCard(@js(route('id-card.show')))).catch(error => window.alert(error.message))"
                                class="inline-flex items-center justify-center gap-2 rounded-lg border border-indigo-200 bg-indigo-50 px-4 py-2.5 text-sm font-semibold text-indigo-700 hover:bg-indigo-100"><x-heroicon-o-arrow-down-tray
                                    class="h-5 w-5" />Download ID Card</button>
                        </div>
                    </section>
