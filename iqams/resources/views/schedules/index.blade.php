            ...
            <div @click.outside="showCreateModal = false"
                ...
                <form method="POST" action="{{ route('schedules.store') }}"
                    ...
                    <div class="mb-4">
                        ...
                        <x-admin-lookup-field :endpoint="route('admin.lookups.subjects')" name="subject_id" model="createSubjectId"
                            :selected="old('_form') === 'create' ? old('subject_id') : null" placeholder="Search subjects..." empty-label="-- Select Subject --" />
                        @error('subject_id')
                    ...
                    <div class="mb-4">
                        ...
                        <x-admin-lookup-field :endpoint="route('admin.lookups.instructors')" name="instructor_id" model="createInstructorId"
                            :selected="old('_form') === 'create' ? old('instructor_id') : null" placeholder="Search instructors..."
                    ...
                    <div class="mb-4">
                        ...
                        <x-admin-lookup-field :endpoint="route('admin.lookups.sections')" name="section_id" model="createSectionId"
                            :selected="old('_form') === 'create' ? old('section_id') : null" placeholder="Search sections..." empty-label="-- Select Section --" />
                        @error('section_id')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4 md:col-span-2">
                        ...
                        <div class="mb-2 flex flex-wrap gap-2">
                            <button type="button" @click="applyPreset(createDays, 'mwf')"
                                class="rounded-md border border-indigo-200 px-3 py-1.5 text-xs font-semibold text-indigo-700 hover:bg-indigo-50">MWF</button>
                            <button type="button" @click="applyPreset(createDays, 'tth')"
                                class="rounded-md border border-indigo-200 px-3 py-1.5 text-xs font-semibold text-indigo-700 hover:bg-indigo-50">TTH</button>
                            <button type="button" @click="applyPreset(createDays, 'weekdays')"
                                class="rounded-md border border-indigo-200 px-3 py-1.5 text-xs font-semibold text-indigo-700 hover:bg-indigo-50">MON-FRI</button>
                            <button type="button" @click="createDays.splice(0)"
                                class="rounded-md border border-gray-200 px-3 py-1.5 text-xs font-semibold text-gray-600 hover:bg-gray-50">CUSTOM</button>
                        </div>
                        <div class="grid grid-cols-4 gap-2 sm:grid-cols-7">
                            @foreach (['monday' => 'MON', 'tuesday' => 'TUE', 'wednesday' => 'WED', 'thursday' => 'THU', 'friday' => 'FRI', 'saturday' => 'SAT', 'sunday' => 'SUN'] as $day => $short)
                                <label
                                    class="cursor-pointer rounded-md border px-2 py-2 text-center text-xs font-semibold transition"
                                    :class="createDays.includes('{{ $day }}') ?
                                        'border-indigo-600 bg-indigo-600 text-white' :
                                        'border-gray-300 bg-white text-gray-600 hover:border-indigo-300'">
                                    <input type="checkbox" name="days[]" value="{{ $day }}"
                                        x-model="createDays" class="sr-only">
                                    {{ $short }}
                                </label>
                            @endforeach
                        </div>
                        <p class="mt-2 text-xs text-gray-500">Selected days: <span
                                x-text="selectedDayNames(createDays) || 'None'"></span></p>
                        @error('days')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                        @error('days.*')
            ...
            <div class="flex min-h-full items-center justify-center">
                <div @click.outside="editModal.show = false"
                    ...
                    <form method="POST" :action="'{{ url('schedules') }}/' + editModal.id"
                        class="grid gap-x-4 md:grid-cols-2" data-password-confirmation-required>
                        @csrf
                        @method('PUT')
                        ...
                        @if ($errors->any() && old('_form') === 'edit')
                            <div
                                class="mb-4 rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700 md:col-span-2">
                        ...
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Subject</label>
                            <x-admin-lookup-field :endpoint="route('admin.lookups.subjects')" name="subject_id" model="editModal.subject_id"
                                :selected="old('_form') === 'edit' ? old('subject_id') : null" placeholder="Search subjects..." empty-label="-- Select Subject --"
                        ...
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Instructor</label>
                            <x-admin-lookup-field :endpoint="route('admin.lookups.instructors')" name="instructor_id"
                                model="editModal.instructor_id" :selected="old('_form') === 'edit' ? old('instructor_id') : null" placeholder="Search instructors..."
                                empty-label="-- Select Instructor --" lookup-key="schedule-edit" />
                            @if (old('_form') === 'edit')
                                @error('instructor_id')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                        ...
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Section</label>
                            <x-admin-lookup-field :endpoint="route('admin.lookups.sections')" name="section_id" model="editModal.section_id"
                                :selected="old('_form') === 'edit' ? old('section_id') : null" placeholder="Search sections..." empty-label="-- Select Section --"
                                lookup-key="schedule-edit" />
                            @if (old('_form') === 'edit')
                                @error('section_id')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div x-show="editModal.recurring_days.length > 1"
                            class="mb-3 rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-2.5 md:col-span-2">
                            <label class="flex cursor-pointer items-start gap-2.5">
                                <input type="hidden" name="apply_to_recurring" value="0">
                                <input type="checkbox" name="apply_to_recurring" value="1"
                                    x-model="editModal.apply_to_recurring" @change="toggleRecurringEdit()"
                                    class="mt-0.5 rounded border-indigo-300 text-indigo-600 focus:ring-indigo-500">
                                <span>
                                    <span class="block text-sm font-semibold text-indigo-900">Apply changes to all
                                        recurring schedules</span>
                                    <span class="mt-1 block text-xs leading-5 text-indigo-700">
                                        This schedule is part of <strong
                                            x-text="dayPattern(editModal.recurring_days)"></strong>.
                                        <span x-show="editModal.apply_to_recurring">Changes will apply to <span
                                                x-text="selectedDayNames(editModal.recurring_days)"></span>.</span>
                                        <span x-show="!editModal.apply_to_recurring">Only <span
                                                x-text="dayNames[editModal.original_day]"></span> will be
                                            changed.</span>
                                    </span>
                                </span>
                            </label>
                        ...
                        <div class="mb-4 md:col-span-2">
                            ...
                            <div x-show="!editModal.apply_to_recurring" class="mb-2 flex flex-wrap gap-2">
                                <button type="button" @click="applyPreset(editModal.days, 'mwf')"
                                    class="rounded-md border border-indigo-200 px-3 py-1.5 text-xs font-semibold text-indigo-700 hover:bg-indigo-50">MWF</button>
                                <button type="button" @click="applyPreset(editModal.days, 'tth')"
                                    class="rounded-md border border-indigo-200 px-3 py-1.5 text-xs font-semibold text-indigo-700 hover:bg-indigo-50">TTH</button>
                                <button type="button" @click="applyPreset(editModal.days, 'weekdays')"
                                    class="rounded-md border border-indigo-200 px-3 py-1.5 text-xs font-semibold text-indigo-700 hover:bg-indigo-50">MON-FRI</button>
                                <button type="button" @click="editModal.days.splice(0)"
                                    class="rounded-md border border-gray-200 px-3 py-1.5 text-xs font-semibold text-gray-600 hover:bg-gray-50">CUSTOM</button>
                            </div>
                            <div class="grid grid-cols-4 gap-2 sm:grid-cols-7">
                                @foreach (['monday' => 'MON', 'tuesday' => 'TUE', 'wednesday' => 'WED', 'thursday' => 'THU', 'friday' => 'FRI', 'saturday' => 'SAT', 'sunday' => 'SUN'] as $day => $short)
                                    <label
                                        class="rounded-md border px-2 py-2 text-center text-xs font-semibold transition"
                                        :class="[
                                            editModal.days.includes('{{ $day }}') ?
                                            'border-indigo-600 bg-indigo-600 text-white' :
                                            'border-gray-300 bg-white text-gray-600 hover:border-indigo-300',
                                            editModal.apply_to_recurring ? 'cursor-default opacity-80' :
                                            'cursor-pointer'
                                        ]">
                                        <input type="checkbox" name="days[]" value="{{ $day }}"
                                            x-model="editModal.days" :disabled="editModal.apply_to_recurring"
                                            class="sr-only">
                                        {{ $short }}
                                    </label>
                                @endforeach
                            ...
                            <template x-if="editModal.apply_to_recurring">
                                <div>
                                    <template x-for="day in editModal.days" :key="'day-' + day"><input
                                            type="hidden" name="days[]" :value="day"></template>
                                </div>
                            </template>
                            <p class="mt-2 text-xs text-gray-500">Selected days: <span
                                    x-text="selectedDayNames(editModal.days) || 'None'"></span></p>
                            @if (old('_form') === 'edit')
                                @error('days')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
