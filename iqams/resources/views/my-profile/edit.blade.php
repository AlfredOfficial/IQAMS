            ...
            <header class="bg-white border-b border-gray-200">
                <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
                    <a href="{{ route($backRoute) }}"
                        class="text-sm text-gray-500 hover:text-gray-800 flex items-center gap-1.5">
            ...
            <div class="border-b border-gray-100 p-6 sm:p-8">
                <div class="flex flex-col gap-6 sm:flex-row sm:items-center">
                    <div x-data="{ preview: @js($user->avatar_url), objectUrl: null }" class="flex items-center gap-5">
                        <div
                            class="relative h-24 w-24 shrink-0 overflow-hidden rounded-full ring-4 ring-white shadow-md">
                            <img x-show="preview" :src="preview" alt="Profile photo preview"
                                class="h-full w-full object-cover">
                            <div x-show="!preview"
                                class="flex h-full w-full items-center justify-center text-3xl font-semibold {{ $accent['avatar'] }}">
                        ...
                        <div>
                            <label for="avatar"
                                class="inline-flex cursor-pointer items-center rounded-md bg-gray-900 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-gray-700 focus-within:ring-2 focus-within:ring-indigo-500 focus-within:ring-offset-2">
                                Change photo
                            </label>
                    ...
                    <div class="sm:ml-auto sm:text-right">
                        ...
                        <div class="mt-2 flex items-center gap-2 sm:justify-end">
                            ...
                            <span
                                class="rounded-full px-2.5 py-1 text-xs font-semibold {{ $user->isAccountActive() ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">{{ ucfirst($user->status) }}</span>
                        </div>
                        @unless ($user->isAccountActive())
                            <p class="mt-2 text-sm text-red-600">Attendance is disabled. Please contact the administrator.
                            </p>
                        @endunless
                    </div>
                </div>
                    ...
                    <div class="mt-6 space-y-5">
                            ...
                            <div class="grid gap-4 sm:grid-cols-3">
                                <div>
                                    ...
                                    <p class="mt-1 rounded-md bg-gray-50 px-3 py-2 text-sm text-gray-700">
                                        {{ $user->student?->student_no ?? '—' }}</p>
                                </div>
                                <div>
                                    <x-input-label value="Course" />
                                    <p class="mt-1 rounded-md bg-gray-50 px-3 py-2 text-sm text-gray-700">
                                        {{ $user->student?->course?->course_code ?? '—' }}</p>
                                </div>
                                <div>
                                    <x-input-label value="Section" />
                                    <p class="mt-1 rounded-md bg-gray-50 px-3 py-2 text-sm text-gray-700">
                                        {{ $user->student?->section?->section_name ?? '—' }}</p>
                                </div>
                            </div>
                        ...
                        @elseif ($roleName === 'staff')
                            <div>
                                <x-input-label value="Employee No." />
                                <p class="mt-1 rounded-md bg-gray-50 px-3 py-2 text-sm text-gray-700">
                                    {{ $user->nonTeachingStaff?->employee_no ?? '—' }}</p>
                            </div>
                        @endif
                    </div>
                </section>
