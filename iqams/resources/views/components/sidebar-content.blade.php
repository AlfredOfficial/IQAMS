...
<div class="flex h-20 shrink-0 items-center border-b border-gray-200 px-4"
    ...
    <a href="{{ route('dashboard') }}" class="flex items-center gap-2 min-w-0"
        aria-label="{{ config('app.name', 'IQAMS') }} home">
        ...
        @else
            <span x-show="!sidebarCollapsed" x-cloak
                class="font-semibold text-gray-800 truncate">{{ config('app.name', 'IQAMS') }}</span>
        @endif
    </a>

    @if ($collapsible)
        <button @click="sidebarCollapsed = !sidebarCollapsed" x-show="!sidebarCollapsed" x-cloak type="button"
...
<nav data-sidebar-nav class="flex-1 overflow-y-auto px-2 py-4 space-y-1" aria-label="Sidebar">
    <x-sidebar-link :href="route('dashboard')" :active="request()->routeIs(
        ...
        'student.dashboard',
        'staff.dashboard',
    )" :collapsible="$collapsible" label="Dashboard">
        <x-slot name="icon">
    ...
    @if ($roleName === 'instructor')
        ...
        'my-profile.edit' => 'Profile',
            ...
            <x-sidebar-link :href="route($routeName)" :active="request()->routeIs($routeName)" :collapsible="$collapsible" :label="$label">
                <x-slot name="icon"><svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
    ...
    @if ($roleName === 'admin')
        <x-sidebar-link :href="route('attendance-scanner.index')" :active="request()->routeIs('attendance-scanner.*')" :collapsible="$collapsible" target="_self" label="QR Scanner">
            <x-slot name="icon">
                <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                    aria-hidden="true">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M3 7V5a2 2 0 012-2h2m10 0h2a2 2 0 012 2v2M3 17v2a2 2 0 002 2h2m10 0h2a2 2 0 002-2v-2M7 7h3v3H7V7zm7 0h3v3h-3V7zM7 14h3v3H7v-3zm7 0h3v3h-3v-3z" />
                </svg>
            </x-slot>
        </x-sidebar-link>

        <x-sidebar-link :href="route('attendance-logs.index')" :active="request()->routeIs('attendance-logs.*')" :collapsible="$collapsible" label="Attendance Logs">
            <x-slot name="icon">
        ...
        <x-sidebar-link :href="route('instructors.index')" :active="request()->routeIs('instructors.*')" :collapsible="$collapsible" label="Manage Instructor">
            <x-slot name="icon">
                <x-heroicon-o-users class="w-5 h-5 shrink-0" aria-hidden="true" />
            </x-slot>
        </x-sidebar-link>
        <x-sidebar-link :href="route('students.index')" :active="request()->routeIs('students.*')" :collapsible="$collapsible" label="Manage Student">
            <x-slot name="icon">
                <x-heroicon-o-academic-cap class="w-5 h-5 shrink-0" aria-hidden="true" />
            </x-slot>
        </x-sidebar-link>
        <x-sidebar-link :href="route('non-teaching-staff.index')" :active="request()->routeIs('non-teaching-staff.*')" :collapsible="$collapsible" label="Manage Staff">
            <x-slot name="icon">
                <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                    aria-hidden="true">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M17 20h5v-2a4 4 0 00-3-3.87M9 20H4v-2a4 4 0 013-3.87m6-8.13a4 4 0 110 8 4 4 0 010-8zM17 8a3 3 0 110 6" />
                </svg>
            </x-slot>
        </x-sidebar-link>
        ...
        <x-sidebar-link :href="route('schedules.index')" :active="request()->routeIs('schedules.*')" :collapsible="$collapsible" label="Manage Schedule">
            <x-slot name="icon">
                <x-heroicon-o-calendar class="w-5 h-5 shrink-0" aria-hidden="true" />
            </x-slot>
        </x-sidebar-link>
        <x-sidebar-link :href="route('departments.index')" :active="request()->routeIs('departments.*')" :collapsible="$collapsible" label="Manage Department">
            <x-slot name="icon">
                <x-heroicon-o-building-office-2 class="w-5 h-5 shrink-0" aria-hidden="true" />
            </x-slot>
        </x-sidebar-link>
        <x-sidebar-link :href="route('courses.index')" :active="request()->routeIs('courses.*')" :collapsible="$collapsible" label="Manage Course">
            <x-slot name="icon">
                <x-heroicon-o-rectangle-group class="w-5 h-5 shrink-0" aria-hidden="true" />
            </x-slot>
        </x-sidebar-link>
        <x-sidebar-link :href="route('subjects.index')" :active="request()->routeIs('subjects.*')" :collapsible="$collapsible" label="Manage Subject">
            <x-slot name="icon">
                <x-heroicon-o-book-open class="w-5 h-5 shrink-0" aria-hidden="true" />
            </x-slot>
        </x-sidebar-link>
        <x-sidebar-link :href="route('sections.index')" :active="request()->routeIs('sections.*')" :collapsible="$collapsible" label="Manage Section">
            <x-slot name="icon">
        ...
        <x-sidebar-link :href="route('office-units.index')" :active="request()->routeIs('office-units.*')" :collapsible="$collapsible" label="Manage Offices">
            <x-slot name="icon"><x-heroicon-o-building-office class="h-5 w-5" /></x-slot>
        </x-sidebar-link>
        <x-sidebar-link :href="route('roles.index')" :active="request()->routeIs('roles.*')" :collapsible="$collapsible" label="Roles">
            <x-slot name="icon">
...
<div class="border-t border-gray-200 p-3 shrink-0" x-data="{ userMenuOpen: false }">
    <button type="button" @click="userMenuOpen = !userMenuOpen"
        ...
        aria-label="Open user menu" :aria-expanded="userMenuOpen">
        @if (Auth::user()->avatar_url)
            <img loading="lazy" width="32" height="32" src="{{ Auth::user()->avatar_thumbnail_url }}"
                alt="" class="w-8 h-8 rounded-full object-cover shrink-0">
        @else
            <div
                class="w-8 h-8 rounded-full bg-indigo-100 text-indigo-700 flex items-center justify-center text-sm font-medium shrink-0">
        ...
        @if (!$collapsible)
            ...
            <x-heroicon-o-chevron-down class="w-4 h-4 text-gray-400 shrink-0 transition-transform"
                x-bind:class="userMenuOpen ? 'rotate-180' : ''" aria-hidden="true" />
        @else
            <div x-show="!sidebarCollapsed" x-cloak class="min-w-0 text-left flex-1">
                <div class="text-sm font-medium text-gray-800 truncate">{{ Auth::user()->name }}</div>
                <div class="text-xs text-gray-400 truncate">{{ ucfirst($roleName ?? 'No role') }}</div>
            </div>
            <x-heroicon-o-chevron-down x-show="!sidebarCollapsed" x-cloak
                class="w-4 h-4 text-gray-400 shrink-0 transition-transform"
                x-bind:class="userMenuOpen ? 'rotate-180' : ''" aria-hidden="true" />
        @endif
    </button>

    <div x-show="userMenuOpen" x-cloak x-transition:enter="transition ease-out duration-150"
        x-transition:enter-start="opacity-0 -translate-y-1" x-transition:enter-end="opacity-100 translate-y-0"
        x-transition:leave="transition ease-in duration-100" x-transition:leave-start="opacity-100"
        x-transition:leave-end="opacity-0" class="mt-1 space-y-1">
        <x-sidebar-link :href="route($roleName === 'admin' ? 'profile.edit' : 'my-profile.edit')" :active="request()->routeIs($roleName === 'admin' ? 'profile.edit' : 'my-profile.edit')" :collapsible="$collapsible" label="Profile">
            <x-slot name="icon">
