    ...
    <div {{ $attributes->class('relative') }} x-data="{
        ...
        <button type="button" @click="toggle"
            ...
            <span x-show="unread > 0" x-cloak x-text="unread > 99 ? '99+' : unread"
                class="absolute -right-2 -top-2 grid min-h-5 min-w-5 place-items-center rounded-full bg-rose-600 px-1 text-[10px] font-bold text-white ring-2 ring-white"></span>
        </button>
        ...
        <div x-show="open" x-cloak x-transition.origin.top.right @click.outside="open=false"
            ...
            <div class="flex items-center justify-between border-b border-slate-100 px-4 py-3">
                <div>
                    ...
                    <p class="text-xs text-slate-500">Recent request activity</p>
                </div>
                @if ($leaveNotifications->isNotEmpty())
                    <a href="{{ route($leaveIndexRoute) }}"
            ...
            <div class="max-h-96 overflow-y-auto divide-y divide-slate-100">
                @forelse($leaveNotifications as $notification)
                    @php
                        ...
                        $message = match ($event) {
                            'submitted' => Auth::user()->isAdmin()
                                ? $data['requester_name'] . ' submitted a leave request.'
                                : 'Your leave request was submitted.',
                            'approved' => 'Your leave request was approved.',
                            'rejected' => 'Your leave request was declined.',
                            'cancelled' => Auth::user()->isAdmin()
                                ? $data['requester_name'] . ' cancelled a leave request.'
                                : 'Your leave request was cancelled.',
                            default => 'A leave request was updated.',
                        };
                    @endphp
                    <a href="{{ $data['url'] ?? '#' }}"
                        class="block px-4 py-3 transition hover:bg-slate-50 {{ $notification->read_at ? '' : 'bg-blue-50/60' }}">
                        <div class="flex items-start justify-between gap-3">
                            <p class="text-sm font-medium leading-5 text-slate-800">{{ $message }}</p><span
                                class="shrink-0 rounded-md px-2 py-0.5 text-[10px] font-semibold capitalize {{ $statusClass }}">{{ $status }}</span>
                        </div>
                        <p class="mt-1 text-xs text-slate-500">{{ $data['leave_type'] ?? 'Leave' }} ·
                            {{ \Illuminate\Support\Carbon::parse($data['start_date'])->format('M j') }}–{{ \Illuminate\Support\Carbon::parse($data['end_date'])->format('M j, Y') }}
                        </p>
                        @if (!empty($data['review_notes']))
                            <p class="mt-1 truncate text-xs text-slate-500">Note: {{ $data['review_notes'] }}</p>
                        @endif
                        <p class="mt-1 text-[11px] text-slate-400">{{ $notification->created_at->diffForHumans() }}</p>
                    </a>
                @empty
                    <div class="px-5 py-10 text-center"><x-heroicon-o-bell class="mx-auto h-8 w-8 text-slate-300" />
                        <p class="mt-2 text-sm text-slate-500">No leave notifications yet.</p>
                    </div>
                @endforelse
            </div>
        </div>
