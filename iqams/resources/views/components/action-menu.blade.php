...
<div class="relative inline-flex" x-data="{
    ...
    <button type="button" x-ref="trigger" @click="toggle()"
        ...
        aria-label="Open actions menu" :aria-expanded="open">
        <x-heroicon-m-ellipsis-vertical class="h-5 w-5" aria-hidden="true" />
    </button>

    <template x-teleport="body">
        <div x-show="open" x-ref="menu" x-cloak x-transition.opacity.duration.100ms @click.outside="open = false"
            ...
            <div @click="open = false"
                class="[&>button]:block [&>button]:w-full [&>button]:px-4 [&>button]:py-2.5 [&>button]:text-left [&>button]:text-gray-700 [&>button:hover]:bg-gray-50">
                {{ $edit }}
            </div>
            @isset($reset)
                <div @click="open = false"
                    class="[&>form]:block [&>form>button]:block [&>form>button]:w-full [&>form>button]:px-4 [&>form>button]:py-2.5 [&>form>button]:text-left [&>form>button]:text-gray-700 [&>form>button:hover]:bg-gray-50">
                    {{ $reset }}
                </div>
            @endisset
            @isset($qr)
            ...
            <form method="POST" action="{{ $toggleAction }}"
                @if ($requiresPasswordConfirmation) data-password-confirmation-required @endif>
                @csrf
                @method('PATCH')
            ...
            <form method="POST" action="{{ $deleteAction }}"
                onsubmit='return confirm({{ Illuminate\Support\Js::from("Deactivate {$deleteName}? Their login will be disabled and historical attendance will be retained.") }})'
                @if ($requiresPasswordConfirmation) data-password-confirmation-required @endif>
                @csrf
                @method('DELETE')
                <button type="submit"
                    class="block w-full px-4 py-2.5 text-left font-medium text-red-600 hover:bg-red-50">Deactivate /
                    Archive</button>
            </form>
        </div>
    </template>
