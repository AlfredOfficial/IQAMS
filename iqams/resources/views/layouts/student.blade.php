...
<div class="min-h-screen lg:flex">
 ...
 <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-50 flex w-60 flex-col border-r border-[#18504e] bg-[#093f3d] text-white transition-transform lg:sticky lg:top-0 lg:h-screen lg:translate-x-0">
  ...
  <form method="POST" action="{{ route('logout') }}" class="border-t border-white/10 p-3">@csrf<button class="flex h-10 w-full items-center gap-3 px-3 text-sm text-teal-50/70 hover:bg-white/5 hover:text-white"><x-heroicon-o-arrow-right-start-on-rectangle class="h-[18px] w-[18px]" />Logout</button></form>
 </aside>
 <div class="min-w-0 flex-1">
  <header class="sticky top-0 z-30 flex min-h-[76px] items-center border-b border-slate-200 bg-white px-4 sm:px-7 lg:px-9"><button @click="sidebarOpen=true" class="mr-3 p-2 text-slate-600 lg:hidden" aria-label="Open navigation"><x-heroicon-o-bars-3 class="h-5 w-5" /></button><div class="min-w-0"><h1 class="truncate text-xl font-semibold text-slate-900 sm:text-2xl">{{ $title }}</h1><p class="hidden text-sm text-slate-500 sm:block">{{ $descriptions[$title] ?? 'IQAMS Student Portal' }}</p></div>
    <div class="relative ml-auto"><button @click="userMenuOpen=!userMenuOpen" class="flex items-center gap-3 border-l border-slate-200 pl-4 text-left sm:pl-6"><span class="hidden max-w-52 text-right sm:block"><span class="block truncate text-sm font-semibold">{{ $portalStudent?->fullName() ?? $portalUser->name }}</span><span class="block text-xs text-slate-500">Student</span></span><span class="h-9 w-9 overflow-hidden rounded-full bg-teal-100 text-teal-800">@if($portalUser->avatar_thumbnail_url)<img loading="lazy" width="36" height="36" src="{{ $portalUser->avatar_thumbnail_url }}" alt="Profile photo" class="h-full w-full object-cover">@else<span class="grid h-full place-items-center text-sm font-semibold">{{ $initials }}</span>@endif</span><x-heroicon-o-chevron-down class="hidden h-4 w-4 text-slate-400 sm:block" /></button><div x-show="userMenuOpen" x-cloak @click.outside="userMenuOpen=false" class="absolute right-0 mt-3 w-52 border border-slate-200 bg-white py-1 shadow-lg"><a href="{{ route('student.profile') }}" class="block px-4 py-2.5 text-sm hover:bg-slate-50">Profile</a><a href="{{ route('student.settings') }}" class="block px-4 py-2.5 text-sm hover:bg-slate-50">Settings</a></div></div>
  </header>
