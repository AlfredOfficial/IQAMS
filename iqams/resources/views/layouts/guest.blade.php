    ...
    <main class="grid min-h-screen lg:grid-cols-2">
        <section class="relative hidden overflow-hidden bg-slate-950 px-12 py-12 text-white lg:flex lg:flex-col">
            <div class="absolute inset-0 opacity-90"
                style="background-image: radial-gradient(circle at 15% 20%, rgba(20, 184, 166, .34), transparent 30%), radial-gradient(circle at 80% 80%, rgba(59, 130, 246, .35), transparent 34%);">
            </div>
            ...
            <div class="relative z-10 my-auto max-w-lg">
                <span
                    class="mb-6 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1.5 text-xs font-medium text-teal-100 backdrop-blur">
                    <span class="h-1.5 w-1.5 rounded-full bg-teal-300"></span>
                    Attendance made simple
                </span>
                <h1 class="text-4xl font-bold leading-tight tracking-tight xl:text-5xl">Every presence tells a story.
                </h1>
                <p class="mt-5 max-w-md text-base leading-7 text-slate-300">Manage attendance, stay on schedule, and
                    keep your academic community connected in one place.</p>
            </div>
        ...
        <section class="flex min-h-screen items-center justify-center px-5 py-10 sm:px-8">
            <div class="w-full max-w-md">
                ...
                {{ $slot }}
                <p class="mt-10 text-center text-xs text-slate-400">&copy; {{ date('Y') }} IQAMS &middot; Integrated
                    QR-Code Attendance Monitoring System</p>
            </div>
        </section>
    </main>
