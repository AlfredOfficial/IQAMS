    ...
    <div class="py-8"
            ...
            <div class="overflow-hidden rounded-lg bg-white shadow-sm">
                ...
                <table class="w-full text-left text-sm">
                    ...
                    @forelse($officeUnits as $unit)
                        <tr><td class="px-6 py-3 font-semibold">{{ $unit->code }}</td><td class="px-6 py-3">{{ $unit->name }}</td><td class="px-6 py-3">{{ $unit->is_active ? 'Active' : 'Inactive' }}</td><td class="px-6 py-3">{{ $unit->staff_count }}</td><td class="px-6 py-3 text-right"><x-record-action-menu><button type="button" @click="edit={show:true,id:{{ $unit->id }},code:@js($unit->code),name:@js($unit->name),is_active:{{ $unit->is_active ? 'true' : 'false' }}}" class="text-indigo-600 hover:text-indigo-800">Edit</button><button type="button" @click="deleteModal={show:true,id:{{ $unit->id }},name:@js($unit->name)}" class="!text-red-600 hover:!text-red-700">Delete</button></x-record-action-menu></td></tr>
            ...
            <div @click.outside="create=false" class="w-full max-w-md rounded-lg bg-white p-6">
                ...
                <form method="POST" action="{{ route('office-units.store') }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input name="code" value="{{ old('code') }}" placeholder="e.g. REG" class="w-full rounded border-gray-300">
                    </div>
                    <div>
                        <label class="mb-1 block text-sm font-medium text-gray-700">Office/Unit Name</label>
                        <input name="name" value="{{ old('name') }}" placeholder="Official name" class="w-full rounded border-gray-300">
                    </div>
        ...
        <div x-show="edit.show" x-cloak class="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4">
            <div @click.outside="edit.show=false" class="w-full max-w-md rounded-lg bg-white p-6">
                ...
                <form method="POST" :action="'{{ url('office-units') }}/'+edit.id" class="space-y-4" data-password-confirmation-required>
                    @csrf
                    @method('PUT')
                    <div>
                        <label class="mb-1 block text-sm font-medium text-gray-700">Office Code</label>
        ...
        <div x-show="deleteModal.show" x-cloak class="fixed inset-0 z-50 flex items-center justify-center px-4" style="background: rgba(0,0,0,0.4);">
            <div @click.outside="deleteModal.show = false" class="w-full max-w-sm rounded-lg bg-white p-6 shadow-xl">
                ...
                <form method="POST" :action="'{{ url('office-units') }}/' + deleteModal.id" data-password-confirmation-required>
                    @csrf
                    @method('DELETE')
