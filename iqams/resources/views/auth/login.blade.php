    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-5">
        ...
        <div>
            ...
            <div class="relative">
                <svg class="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-400"
                    ...
                    <path stroke-linecap="round" d="M8 9h8M8 13h5M8 17h4" />
                </svg>
                <x-text-input id="user_id"
                    class="block w-full rounded-xl border-slate-200 py-3 pl-11 pr-4 text-sm shadow-sm placeholder:text-slate-400 focus:border-teal-500 focus:ring-teal-500"
        ...
        <div>
            <div class="mb-2 flex items-center justify-between">
                ...
                @if (Route::has('password.request'))
                    <a class="text-sm font-semibold text-teal-600 transition hover:text-teal-700 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 rounded"
        ...
        <label for="remember_me" class="inline-flex cursor-pointer items-center gap-2.5 text-sm text-slate-600">
            <input id="remember_me" type="checkbox"
                class="rounded border-slate-300 text-teal-600 shadow-sm focus:ring-teal-500" name="remember">
