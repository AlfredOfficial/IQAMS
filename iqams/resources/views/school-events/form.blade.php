<form method="POST" :action="formAction" class="space-y-5">
    ...
    <div><label class="text-sm font-medium text-gray-700">Description</label><textarea name="description" x-model="form.description" rows="3" class="mt-1 block w-full rounded-md border-gray-300"></textarea></div>
    ...
    <div x-show="form.target_scope === 'sections'" x-cloak><label class="text-sm font-medium text-gray-700">Sections</label><select name="section_ids[]" x-model="form.section_ids" multiple size="6" class="mt-1 block w-full rounded-md border-gray-300">@foreach($sections as $section)<option value="{{ $section->id }}">{{ $section->section_name }} — {{ $section->school_year }} {{ $section->semester }}</option>@endforeach</select><p class="mt-1 text-xs text-gray-500">Use Ctrl/Cmd to select more than one.</p></div>
    <div x-show="form.target_scope === 'schedules'" x-cloak><label class="text-sm font-medium text-gray-700">Schedules overlapping this event</label><select name="schedule_ids[]" x-model="form.schedule_ids" multiple size="7" class="mt-1 block w-full rounded-md border-gray-300">@foreach($schedules as $schedule)<option value="{{ $schedule->id }}">{{ ucfirst($schedule->day) }} {{ \Illuminate\Support\Carbon::parse($schedule->start_time)->format('g:i A') }} — {{ $schedule->subject?->subject_code }} / {{ $schedule->section?->section_name }}</option>@endforeach</select></div>
