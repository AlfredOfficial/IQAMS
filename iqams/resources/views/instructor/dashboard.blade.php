@php
    $periods = [
        ...
        'lunch_out' => ['MORNING', 'Time-Out'],
        'afternoon_in' => ['AFTERNOON', 'Time-In'],
        'final_out' => ['AFTERNOON', 'Time-Out'],
    ];
    $progressPeriods = [
    ...
    $icons = [
        'attendance' =>
            'M9 12l2 2 4-4m5-4v14a2 2 0 01-2 2H6a2 2 0 01-2-2V6a2 2 0 012-2h2m4-1h4a1 1 0 011 1v2H9V4a1 1 0 011-1h2z',
        'history' => 'M12 8v4l3 2m6-2a9 9 0 11-18 0 9 9 0 0118 0z',
        'summary' => 'M4 19h16M7 16V9m5 7V5m5 11v-4',
        'schedule' => 'M8 7V3m8 4V3M5 11h14M5 5h14v16H5z',
        'issues' => 'M12 9v4m0 4h.01M10.3 3.6L2.6 17a2 2 0 001.7 3h15.4a2 2 0 001.7-3L13.7 3.6a2 2 0 00-3.4 0z',
        'profile' => 'M5.1 17.8a9 9 0 1113.8 0M15 11a3 3 0 11-6 0 3 3 0 016 0z',
    ];
    $chartWidth = 760;
    $chartHeight = 230;
    $chartTop = 16;
    ...
    $chartPoints = $chartDays->map(function (array $day, int $index) use (
        $chartDays,
        $chartLeft,
        $chartPlotWidth,
        $chartTop,
        ...
        $x =
            $chartLeft +
            ($chartDays->count() > 1 ? ($index * $chartPlotWidth) / ($chartDays->count() - 1) : $chartPlotWidth / 2);
        ...
        @unless (Auth::user()->isAccountActive())
            <div class="rounded-2xl border border-red-200 bg-red-50 px-5 py-4 text-red-800">
                <div class="flex items-center gap-3"><span
                        class="rounded-full bg-red-100 px-2.5 py-1 text-xs font-bold">Inactive</span>
                    <p class="text-sm font-semibold">Attendance unavailable. Your account is inactive. Please contact the
                        administrator.</p>
                </div>
            </div>
        @endunless
            ...
            <div class="space-y-4">
                <section class="rounded-xl border border-gray-200 bg-white px-4 py-4 sm:px-5">
                    ...
                    <div class="flex flex-wrap items-center justify-between gap-2">
                        <div class="flex items-baseline gap-2">
                            <p data-instructor-progress-count class="text-sm font-bold text-gray-900">
                                {{ $today['completedPeriods'] }} of 4 completed</p>
                            <p data-instructor-progress-percent class="text-sm font-semibold text-emerald-700">
                                {{ $today['progressPercentage'] }}%</p>
                        </div>
                        <span data-instructor-status
                    ...
                    <div class="mt-3 h-2.5 w-full overflow-hidden rounded-full bg-gray-200" role="progressbar"
                        ...
                        aria-valuenow="{{ $today['progressPercentage'] }}" data-instructor-progress-track>
                        <div data-instructor-progress-bar
                        ...
                        @foreach ($progressPeriods as $key => $label)
                            ...
                            <div data-instructor-milestone="{{ $key }}"
                                ...
                                <span data-instructor-milestone-label
                                    class="min-w-0 text-sm font-semibold {{ $log ? 'text-emerald-700' : 'text-gray-600' }}">{{ $label }}</span>
                            </div>
                        @endforeach
                    </div>
                ...
                <div class="grid gap-4 lg:grid-cols-[.85fr_1.15fr]">
                    <section class="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
                        ...
                        <div class="mt-3 divide-y divide-slate-100">
                            @foreach ($periods as $key => [$session, $label])
                                @php($log = $today['events'][$key])
                                <div class="flex items-center gap-3 py-2.5"><span
                                        class="grid h-7 w-7 place-items-center rounded-full {{ $log ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-400' }}">◷</span>
                                    <p class="flex-1 text-xs font-semibold">{{ ucfirst(strtolower($session)) }}
                                        {{ $label }}</p><span id="detail-{{ $key }}"
                                        class="text-right text-[11px] font-medium {{ in_array($log?->punctuality_status, ['late', 'early_out']) ? 'text-rose-600' : ($log ? 'text-emerald-600' : 'text-slate-400') }}">{{ $log? str($log->punctuality_status ?? 'on_time')->replace('_', ' ')->title(): 'Not Yet Recorded' }}</span>
                                </div>
                            @endforeach
                        </div>
                    </section>
                    <section class="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
                        ...
                        <div class="mt-2 flex items-end justify-between rounded-lg border border-slate-100 px-3 py-2">
                            <div>
                                <p class="text-[10px] text-slate-500">Total Hours</p>
                                <p data-stat="hours" class="text-lg font-extrabold text-slate-950">
                                    {{ intdiv($totals['totalMinutes'], 60) }}h {{ $totals['totalMinutes'] % 60 }}m</p>
                            </div><span class="text-3xl text-blue-300">◷</span>
                        </div>
                    </section>
                </div>
                ...
                <section class="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
                    <div class="flex items-center justify-between gap-3">
                        <div>
                            ...
                            <p class="mt-1 text-[11px] text-slate-500">Daily completion of your four attendance periods
                            </p>
                        </div>
                        <a href="{{ route('instructor.summary') }}"
                    ...
                    @if ($chartHasData)
                        <div class="mt-4 overflow-x-auto rounded-xl border border-slate-100 bg-slate-50/40 px-2 pt-2">
                            <svg class="h-64 min-w-[680px] w-full"
                                ...
                                <desc id="instructor-attendance-chart-description">Attendance completion percentage for
                                    each working day this month. One hundred percent means all four attendance periods
                                    were recorded.</desc>
                                @foreach ([100, 75, 50, 25, 0] as $gridValue)
                                    @php($gridY = $chartTop + ((100 - $gridValue) * $chartPlotHeight) / 100)
                                    <line x1="{{ $chartLeft }}" y1="{{ $gridY }}"
                                        x2="{{ $chartWidth - $chartRight }}" y2="{{ $gridY }}" stroke="#e2e8f0"
                                        stroke-width="1" />
                                    <text x="{{ $chartLeft - 10 }}" y="{{ $gridY + 4 }}" text-anchor="end"
                                        fill="#64748b" font-size="11">{{ $gridValue }}%</text>
                                @endforeach
                                @foreach ($chartSegments as $segment)
                                    <polyline points="{{ $segment }}" fill="none" stroke="#2563eb"
                                ...
                                @foreach ($chartPoints as $point)
                                    @if ($point['value'] !== null)
                                        <circle cx="{{ $point['x'] }}" cy="{{ $point['y'] }}" r="4.5"
                                            ...
                                            <title>{{ $point['day']['date']->format('M j') }}: {{ $point['value'] }}%
                                                ({{ $point['day']['status'] }})</title>
                                        </circle>
                                    @endif
                                    @if ($point['showLabel'])
                                        <text x="{{ $point['x'] }}" y="{{ $chartHeight - 9 }}" text-anchor="middle"
                                            fill="#64748b"
                                            font-size="10">{{ $point['day']['date']->format('M j') }}</text>
                                    @endif
                                @endforeach
                            </svg>
                        </div>
                    @else
                        <div
                            class="mt-4 rounded-xl border border-dashed border-slate-200 bg-slate-50 p-8 text-center text-xs text-slate-500">
                            No rated attendance days yet this month.</div>
                    @endif
                    <div class="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-[10px] text-slate-600">
                        <span class="inline-flex items-center gap-1.5"><i
                                class="h-0.5 w-4 rounded-full bg-blue-600"></i>Daily completion</span>
                        @foreach (['#34d399' => 'Complete', '#fbbf24' => 'Incomplete', '#f43f5e' => 'Absent'] as $color => $label)
                            <span class="inline-flex items-center gap-1.5"><i class="h-2 w-2 rounded-full"
                                    style="background-color:{{ $color }}"></i>{{ $label }}</span>
                        @endforeach
                    </div>
                </section>
            </div>
                ...
                <section class="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
                    ...
                    <div class="mt-4 flex items-center gap-4">
                        <div class="h-20 w-20 shrink-0 overflow-hidden rounded-full bg-blue-100 text-blue-800">
                            @if (auth()->user()->avatar_thumbnail_url)
                                <img loading="lazy" width="80" height="80"
                                    src="{{ auth()->user()->avatar_thumbnail_url }}"
                                class="h-full w-full object-cover" alt="{{ $instructor->fullName() }}">@else<div
                                    class="grid h-full place-items-center text-xl font-bold">
                        ...
                        <div class="min-w-0 text-sm leading-6">
                            ...
                            <p class="text-slate-600">Department:
                                {{ $instructor->department?->department_code ?? ($instructor->department?->department_name ?? 'N/A') }}
                            </p>
                    ...
                    <div class="mt-4 text-center">
                        ...
                        <p class="mx-auto mt-2 max-w-[240px] text-[11px] leading-4 text-slate-500">Present this QR code
                            to the dedicated scanner for attendance.</p><button type="button" @click="downloadIdCard"
                            class="mx-auto mt-3 inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-xs font-bold text-white hover:bg-blue-700"><x-heroicon-o-arrow-down-tray
                ...
                <section class="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
                    ...
                    @if ($nextSchedule)
                        <div class="mt-3 border-t border-slate-100 pt-3">
                            <p class="text-sm font-extrabold text-[#10294b]">◷
                                &nbsp;{{ Carbon\Carbon::parse($nextSchedule->start_time)->format('g:i A') }} -
                                {{ Carbon\Carbon::parse($nextSchedule->end_time)->format('g:i A') }}</p>
                            <p class="mt-2 text-xs font-semibold">♙
                                &nbsp;{{ $nextSchedule->subject?->subject_name ?? 'Unnamed subject' }}</p>
                            <p class="mt-2 text-xs text-slate-600">♧
                                &nbsp;{{ $nextSchedule->section?->section_name ?? 'No section' }}</p>
                            <p class="mt-2 text-xs text-slate-600">⌖ &nbsp;Room {{ $nextSchedule->room ?? 'TBD' }}</p>
                    </div>@else<p class="mt-3 rounded-lg bg-slate-50 p-4 text-center text-xs text-slate-500">No
                    ...
                    <div class="mt-3 divide-y divide-slate-100">
                        @forelse($issues as $issue)
                            <div class="flex items-center gap-3 py-3"><span
                                    class="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-orange-100 text-orange-600">△</span>
                                <div class="min-w-0 flex-1">
                                    <p class="truncate text-xs font-bold">
                                        {{ $issue['status'] === 'Absent' ? 'Absent Day' : ($issue['isIncomplete'] ? 'Incomplete Attendance' : ($issue['late'] ? 'Late Time-In' : 'Early Time-Out')) }}
                                    </p>
                                    <p class="mt-1 text-[10px] text-slate-500">{{ $issue['date']->format('F j, Y') }}
                                        · {{ $issue['punctuality'] }}</p>
                                </div><span
                                    class="rounded-lg bg-orange-50 px-2.5 py-1 text-[9px] font-bold text-orange-600">Open</span>
                        </div>@empty<p class="rounded-lg bg-emerald-50 p-4 text-center text-xs text-emerald-700">No
                                attendance issues found.</p>
                        @endforelse
                    </div>
                </section>
            </aside>
        ...
        <section class="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm">
            ...
            <div class="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-6">
                @foreach ([['attendance', 'My Attendance'], ['history', 'Attendance History'], ['summary', 'Monthly Summary'], ['schedule', 'My Teaching Schedule'], ['issues', 'Attendance Issues'], ['profile', 'Profile']] as [$route, $label])
                    <a href="{{ $route === 'profile' ? route('my-profile.edit') : route('instructor.' . $route) }}"
                        class="flex min-h-14 items-center justify-center gap-3 rounded-xl border border-slate-100 px-3 py-2 text-center text-[11px] font-semibold text-[#10294b] transition hover:border-blue-200 hover:bg-blue-50"><svg
                            class="h-6 w-6 shrink-0 text-blue-600" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
