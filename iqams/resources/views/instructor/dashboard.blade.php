@php
    ...
    $statusClasses = [
        'Completed' => 'bg-emerald-100 text-emerald-700', 'Absent' => 'bg-red-100 text-red-700',
    ...
    <div class="grid gap-4 xl:grid-cols-[minmax(0,1.9fr)_minmax(310px,1fr)]">
        <div class="space-y-4">
            ...
            <div class="grid gap-4 lg:grid-cols-[.85fr_1.15fr]">
                ...
                <section class="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm"><div class="flex items-center justify-between"><h2 class="text-xs font-extrabold uppercase text-[#10294b]">Monthly Attendance Overview</h2><a href="{{ route('instructor.summary') }}" class="rounded-lg bg-slate-50 px-2.5 py-1.5 text-[10px] font-semibold">{{ now()->format('F Y') }}⌄</a></div><div class="mt-3 grid grid-cols-3 gap-2 text-center">@foreach([['Attendance Rate','attendance',$totals['percentage'].'%','bg-blue-50 text-blue-700'],['Present','present',$totals['presentDays'].' days','bg-emerald-50 text-emerald-700'],['Absent','absent',$totals['absentDays'].' days','bg-rose-50 text-rose-700'],['Late','late',$totals['lateCount'].' days','bg-orange-50 text-orange-600'],['Early Out','early',$totals['earlyOutCount'].' days','bg-purple-50 text-purple-700'],['Incomplete','incomplete',$totals['incompleteCount'].' days','bg-amber-50 text-amber-700']] as [$label,$key,$value,$class])<div class="rounded-lg p-2 {{ $class }}"><p class="text-[10px] text-slate-600">{{ $label }}</p><p data-stat="{{ $key }}" class="mt-1 text-lg font-extrabold">{{ $value }}</p></div>@endforeach</div><div class="mt-2 flex items-end justify-between rounded-lg border border-slate-100 px-3 py-2"><div><p class="text-[10px] text-slate-500">Total Hours</p><p data-stat="hours" class="text-lg font-extrabold text-slate-950">{{ intdiv($totals['totalMinutes'],60) }}h {{ $totals['totalMinutes']%60 }}m</p></div><span class="text-3xl text-blue-300">◷</span></div></section>
            </div>
